...
class UserController extends Controller
{
    ...
    public  function requestPasswordReset(Request $request, $username){
        ...
        if((count($currentValidPasswordResetInvitations) === 1)){
            ...
            $url = url('/').'/users/' . $userToResetPassword->userid . '/password-reset/'.$currentValidPasswordResetInvitation->invitationid;
            ...
            return response(array('success' => 1, 'faillure' => 0, 'response' => 'Password Reset Invitation resent successfully'), 200);
            ...
            $url = env('HOST_WEB_CLIENT') . '/users/' . $userToResetPassword->userid . '/password-reset/' . $passwordResetInvitation->invitationid;
        ...
        return response(array('success' => 1, 'faillure' => 0, 'response' => 'Password Reset Invitation created successfully'), 200);
    ...
    public function inviteUserToRegister(Request $request){
        ...
        if (count($userRegistrationInvitions) > 0){
            ...
            $url = env('HOST_WEB_CLIENT').'/users/' . $userRegistrationInvition->userid . '/registration-invitations/'.$userRegistrationInvition->tenantid;
